...
object BitmapFileIoUtils {
    ...
    @JvmStatic
    fun readBitmapFromFile(filePath: String, bitmapScale: BitmapScale): Optional<Bitmap> {
        val file = File(filePath)
        val bitmapScaleValue = bitmapScale.value
        if (bitmapScaleValue < 1) {
            return Optional.ofNullable(getScaledImage(filePath, bitmapScaleValue))
        }
    ...
    private fun getScaledImage(filePath: String, scale: Float): Bitmap? {
        val file = File(filePath)
        val options = BitmapFactory.Options()

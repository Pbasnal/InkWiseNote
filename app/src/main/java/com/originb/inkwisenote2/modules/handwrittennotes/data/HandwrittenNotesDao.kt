...
interface HandwrittenNotesDao {
    ...
    val allHandwrittenNotes: MutableList<HandwrittenNoteEntity>
    ...
    fun getHandwrittenNoteForBook(bookId: Long): MutableList<HandwrittenNoteEntity>

    @Query("SELECT * FROM handwritten_notes WHERE book_id IN (:bookIds)")
    fun getHandwrittenNoteForBooks(bookIds: MutableSet<Long>): MutableList<HandwrittenNoteEntity>
    ...
    @Update
    fun updateHandwrittenNotes(handwrittenNoteEntities: MutableList<HandwrittenNoteEntity>): Int

    @Query("DELETE FROM handwritten_notes WHERE note_id IN (:noteIds)")
    fun deleteHandwrittenNotes(noteIds: MutableSet<Long>)

    @Upsert
    fun upsert(note: HandwrittenNoteEntity)
}
